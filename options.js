...
// 既存の save 関数を Promise 対応に
function save(settings) {
  return new Promise(res => {
    chrome.storage.sync.get('cgNavSettings', ({ cgNavSettings }) => {
      void chrome.runtime.lastError; // エラー抑制
      const merged = deepMerge(structuredClone(cgNavSettings || {}), settings);
...
// debouncedAutosave 関数を差分チェック込みで置き換え
const debouncedAutosave = debounce(async () => {
  const base = load._last || structuredClone(DEFAULTS);
  const next = collectSettingsFromDOM(base);

  // JSON文字列で簡易的に差分を比較
  if (JSON.stringify(load._last) !== JSON.stringify(next)) {
    await save(next);
    load._last = next; // 保存後にキャッシュを更新
    showStatus('保存しました');
    // shared.js 経由で可視ラインを即時反映
    if (window.CGTN?.renderViz) {
      window.CGTN.renderViz(next);
    }
  }
}, 300); // 300ms のディレイ
