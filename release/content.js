...
(function () {
    ...
    async function rebuildAndRenderSafely({ forceList = false } = {}, oldSig = null) {
        ...
        try {
            // 1. ロジック再構築
            // silent=true で呼びたいところですが、logic.ts 側が未対応なら
            // updateStatus() が走って一旦数字が出る可能性があります。
            // それでも「止まる」よりはマシなので、普通に呼びます。
            LG.rebuild?.();
            // 2. ★修正: リストを作るかどうか判定
            const kind = SH.getPageInfo?.()?.kind || "other";
            const cfg = SH.getCFG?.() || {};
            // forceList があるか、設定で enabled ならリストを作る
            ...
            if (needList) {
                ...
                if (myGen !== __buildGen)
                    return;
                await LG.renderList?.(true); // true = force
            }
            // 3. ★修正: リストの有無に関わらず、最後に必ず更新！
            // これで Loading... が消えて数字になります
            if (typeof LG.updateStatus === "function") {
                LG.updateStatus();
            }
        }
            ...
            (async () => {
                ...
                if (d.type === "url-change" || d.type === "turn-added") {
                    ...
                    if (d.type === "url-change") {
                        ...
                        console.log("d.type === url-change catch setUiBusy(true,Loding...)");
                        setUiBusy(true, "Loading...");
                        // ★追加: リストが開いているかチェック
                        ...
                        console.log("d.type === url-change else  setUiBusy(true,Loding...)");
                        setUiBusy(true, "Loading...");
                    }
                    clearTimeout(__debTo);
                    __debTo = window.setTimeout(() => {
                        requestAnimationFrame(() => {
                            (async () => {
                                ...
                                try {
                                    ...
                                    console.log("rebuildAndRenderSafely finally setUiBusy(false)");
                                    setUiBusy(false);
                                }
                                catch { }
                            });
