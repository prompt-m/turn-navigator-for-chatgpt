...
(() => {
    ...
    const canUseStorage = () => !!(chrome?.runtime?.id && chrome?.storage?.sync) && !_dying;
    ...
    SH.refreshCurrentChatTitle = function () {
        try {
            ...
            if (id && title && title !== "ChatGPT")
                SH.setChatTitleForId(id, title);
        }
        catch { }
    };
    // shared.js に追記（削除しない）
  ...
  */
    ...
    SH.togglePinByIndex = async function togglePinByIndex(index1, chatId = SH.getChatId?.()) {
        ...
        if (!ok) {
            ...
            try {
                window.CGTN_UI?.toastNearPointer?.(SH.t?.("options.saveFailed") || "Failed to save");
            }
            catch (_) { }
            return false;
        }
        // 付箋バッジ
        try {
            document.dispatchEvent(new CustomEvent("cgtn:pins-updated", { detail: { chatId } }));
        }
        catch { }
        return !!next;
    };
    ...
    try {
        chrome?.storage?.onChanged?.addListener?.((changes, area) => {
            ...
            if (!next.list)
                next.list = structuredClone(DEFAULTS.list);
            CFG = next;
    ...
    SH.migrateStorageIfNeeded = async function () {
        return new Promise((resolve) => {
            chrome.storage.sync.get(null, (raw) => {
                ...
                const hasGarbage = raw.meta !== undefined || raw.pinsByChat !== undefined;
                // v2以上で、かつゴミもなければ終了
    ...
    SH.exportAllData = async function () {
        const raw = await new Promise((r) => chrome.storage.sync.get(null, r));
        const v2Data = migrateV1toV2(raw);
