    ...
    function sendToActive(payload) {
        return new Promise((resolve) => {
            ...
            chrome.tabs.query({ url: urls, active: true, lastFocusedWindow: true }, (tabs) => {
                const t = tabs?.[0];
                if (!t?.id)
                    return resolve({ ok: false, reason: "no-tab" });
  ...
  */
    ...
    function getNormalizedPinsForOptions(cfg) {
        ...
        for (const [cid, recU] of Object.entries(raw)) {
            const rec = recU && typeof recU === "object" ? recU : null;
            if (!rec)
                continue;
            const pinsObj = rec.pins && typeof rec.pins === "object"
                ? rec.pins
                : {};
    ...
    async function renderPinsManager() {
        ...
        if (refreshBtn) {
            refreshBtn.onclick = async () => {
                if (refreshBtn.classList.contains("is-busy"))
                    return;
                setBusy(refreshBtn, true, {
                    onTimeout: () => flashMsgInline?.("pins-msg", "options.refreshTimeout"),
                });
                try {
                    ...
                    if (meta?.ok) {
                        ...
                        if (chatId) {
                            const tr = box?.querySelector(`tr[data-cid="${chatId}"]`);
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        try {
            ...
            document.getElementById("resetList")?.addEventListener("click", () => {
                ...
                if (maxCharsEl)
                    maxCharsEl.value = String(patch.list.maxChars);
                if (fontSizeEl)
                    fontSizeEl.value = String(patch.list.fontSize);
            ...
            document.getElementById("resetAdv")?.addEventListener("click", () => {
                ...
                if (cb)
                    cb.value = String(DEF.centerBias);
                const ep = $inp("eps");
                if (ep)
                    ep.value = String(DEF.eps);
                const lm = $inp("lockMs");
                if (lm)
