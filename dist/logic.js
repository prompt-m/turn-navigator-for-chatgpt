...
(() => {
  ...
  */
    ...
    NS.installAutoSyncForTurns = function installAutoSyncForTurns() {
        ...
        const isStreamingNow = () => !!document.querySelector("button[data-testid='stop-button'], button[aria-label*='Stop']");
        // ---- timers/state
        let kickTo = null; // 通常debounce用
        let streamTo = null; // streaming待ち用（1本）
        ...
        const STREAM_MAX_MS = 30000; // 保険（止まらない時）
        const doRender = () => {
            try {
    ...
    if (chrome.storage?.onChanged && !window.__CGTN_ONCHG_BOUND__) {
        ...
        chrome.storage.onChanged.addListener((changes, areaName) => {
            ...
            // 即時反映（必要なものだけ）
            if (themeChanged)
                window.CGTN_UI.applyTheme({ mode: newThemeMode });
