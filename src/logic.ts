...
(() => {
  ...
  let uploads = 0,
    downloads = 0; // ダウンロードターン数・アップロードターン数
  ...
  let PINS = new Set<string>(); // "turn:1" みたいなキー集合（Setで保持）

  const _delegatedClipBound = new WeakSet<HTMLElement>(); // !!!!
  ...
  NS.updateStatus = function updateStatus() {
    ...
    if (state === "ACTIVE") {
      ...
      // =======================================================
      const sc =
        typeof getTrueScroller === "function"
          ? getTrueScroller()
  ...
  function headNodeOf(article) {
    ...
    const isUser =
      article.matches('[data-message-author-role="user"]') ||
  ...
  function detectAttachmentKinds(root) {
    ...
    const docRe =
      /\.(pdf|md|txt|csv|tsv|docx?|xlsx?|pptx?|js|ts|gs|json|htm|html)$/i;
  ...
  function buildAttachmentLine(root, maxChars) {
    const el = root || document;
    const role =
      (typeof getTurnRole === "function" ? getTurnRole(el) : "unknown") ||
      "unknown";
    ...
    if (names.length) {
      // ローカル小ヘルパ：PDF抽出
      const pickPdfNames = (arr) =>
        (arr || []).filter((n) => /\.pdf(\b|$)/i.test(String(n)));
    ...
    if (hasImg || hasVid) {
      const kind =
        hasImg && hasVid ? T("media") : hasImg ? T("image") : T("video");
  ...
  function _pinsSetFromCFG(cfg) {
    const arr =
      cfg && cfg.list && Array.isArray(cfg.list.pins) ? cfg.list.pins : [];
  ...
  function _savePinsSet(set) {
    ...
    PINS = new Set<string>(Array.from(set, (v) => String(v)));
    //PINS = new Set(set); !!!!
    const cur = SH.getCFG() || {};
  ...
  function bindDelegatedClipPinHandler() {
    ...
    body.addEventListener(
      ...
      async (ev) => {
        ...
        const clipEl = t.closest(".cgtn-clip-pin") as HTMLElement | null;
        ...
        const rowEl = clipEl.closest(".row") as HTMLElement | null;
        if (!rowEl) return;
        ...
        try {
          ...
          targets.forEach((r) => {
            const rr = r as HTMLElement; // !!!!
            if (next) rr.dataset.pin = "1";
            else rr.removeAttribute("data-pin");
  ...
  function refreshPinUIForTurn(turnKey, forcedState) {
    const state =
      typeof forcedState === "boolean" ? forcedState : isPinnedByKey(turnKey);
  ...
  async function bulkSetPins(
    ...
    if (!willChange) {
      SH.addLog?.(
        `[bulkSetPins] no-op (role=${role}, mode=${doPinOn ? "on" : "off"})`,
        "DEBUG",
      );
      return;
    }
    ...
    if (opts?.confirm && hasAnyPin) {
      ...
      SH.addLog?.(
        `bulkPins confirm ${doPinOn ? "ON" : "OFF"} -> ${ok ? "OK" : "Cancel"}`,
      );
      if (!ok) return;
    }
    ...
    try {
      ...
      if (body) {
        ...
        for (const rowEl of rows) {
          ...
          if (!idx1 || !targetSet.has(idx1)) continue; // 対象外のターンは触らない

          // role=全体の時は全行、role=user/asst の時は
          // data-role で既に ST 側で絞り込み済みなので、そのまま適用
          if (doPinOn) rowEl.dataset.pin = "1";
          else rowEl.removeAttribute("data-pin");
  ...
  function pickAllTurns(): HTMLElement[] {
    ...
    if (!list.length) {
      ...
      list = nodes
        .map((n) => (n.closest("article") as HTMLElement | null) || n)
    ...
    list = list
      .map((el) => {
        const art =
          el.tagName === "ARTICLE"
            ? el
            : (el.closest("article") as HTMLElement | null) || el;
        return art;
      })
  ...
  function ensureListBox() {
    ...
    (function bindListEvents(panel) {
      ...
      if (!body || (body as any)._cgtnClickBound) return;
      (body as any)._cgtnClickBound = true;
      ...
      body.addEventListener("click", (e) => {
        ...
        if (pinBtn) {
          ...
          const row = pinBtn.closest(".row") as HTMLElement;
          if (!row) return;
  ...
  NS.renderList = async function renderList(
    ...
    const kind =
      info.kind || (location.pathname.includes("/c/") ? "chat" : "other");
    if (kind !== "chat") {
      LG?.clearListPanelUI?.();
      return;
    }
...
*/
    ...
    (NS as any)._listRenderPending = (NS as any)._listRenderPending || false;
    if ((NS as any)._listRendering) {
      (NS as any)._listRenderPending = true;
      return;
    }
    (NS as any)._listRendering = true;
    ...
    try {
      ...
      if (!LG?.ST?.all?.length) {
        LG?.rebuild?.();
        if (!LG?.ST?.all?.length) return;
      }

      const cidAtStart = cid || SH.getChatId?.();
      const body = panel.querySelector("#cgpt-list-body") as HTMLElement;
      ...
      for (let i = 0; i < totalItems; i += CHUNK_SIZE) {
        ...
        for (const art of chunk) {
          ...
          if (bodyLine) {
            ...
            if (isPinned(art)) row2.dataset.pin = "1";
      ...
      for (let i = 0; i < totalTurnsForOrphan; i++) {
        if (pinsArr[i]) validPinsCountForRender++;
      }
    ...
    `;
      ...
      if (my === _renderTicket && NS._panelOpen) {
        ...
        if (cidAtStart) _lastRenderedChatId = String(cidAtStart);
      }
    } finally {
      // ★描画中フラグを必ず解除
      (NS as any)._listRendering = false;
      // ★保留が立っていたら 1回だけ追いかける
      if ((NS as any)._listRenderPending) {
        (NS as any)._listRenderPending = false;
        setTimeout(() => {
          try {
...
*/
  ...
  function updateListFooterInfo() {
    ...
    let uploads =
      typeof box.uploads === "number" ? box.uploads : Number(NS?.uploads || 0);
    let downloads =
      typeof box.downloads === "number"
        ? box.downloads
  ...
  NS.togglePin = async function (article) {
    ...
    if (body) {
      ...
      rows.forEach((r) => {
        ...
        if (nextState) row.dataset.pin = "1";
        else row.removeAttribute("data-pin");
    ...
    // =======================================================
    SH.savePinsArrAsync(arr, cid).catch((e) =>
      SH.logError("Optimistic save failed", e),
    );
  };
  // --- expose ---
  ...
  function goTop(role) {
    const L =
      role === "user" ? ST.user : role === "assistant" ? ST.assistant : ST.all;
    if (!L?.length) return;
    scrollToHead(L[0]);
  }
  ...
  NS.installAutoSyncForTurns = function installAutoSyncForTurns() {
    ...
    const inComposer = (node: Node | null) => {
