...
(function () {
  ...
  type WaitOptions = {
    cid?: string;
    expectZeroFirst?: boolean;
    idleMs?: number;
    tickMs?: number;
    maxMs?: number;
  };
  async function waitForChatSettled({
    cid,
    expectZeroFirst = false,
    idleMs = 350,
    tickMs = 120,
    maxMs = 12000,
  }: WaitOptions = {}) {
      ...
      const check = () => {
        ...
        if (AUTO_SYNC_OPEN_LIST && SH.isListOpen?.()) {
          try {
            ...
            if (cid) LG?.hydratePinsCache?.(cid);
            else LG?.hydratePinsCache?.();
...
*/
  ...
  function handleUrlChangeMessage() {
    ...
    try {
      ...
      if (wantReopen) {
        waitForChatMain(() => {
          if (mySeq === _navSeq) LG?.setListEnabled?.(true);
        });
      }
    } catch (e) {
        ...
        for (const m of muts) {
          ...
          if (m.type === "childList") {
            ...
            const hit = arr.some((n) => {
              if (!(n instanceof Element)) return false; // ★ここが肝
