...
(() => {
  ...
  window
    ...
    .addEventListener("change", () => {
      ...
      if (currentMode === "auto") NS.applyTheme({ mode: "auto" });
    });
  ...
  function installUI() {
    ...
    else valText = T("nav.sk_enter"); // "[Enter]"
    ...
    if (grpOthers) grpOthers.appendChild(sendKeyDiv);
    // =========================================================
  ...
  function applyLang() {
    ...
    if (skVal) {
      ...
      if (method === "ctrl_enter") key = "nav.sk_ctrl";
      else if (method === "alt_enter") key = "nav.sk_alt";
